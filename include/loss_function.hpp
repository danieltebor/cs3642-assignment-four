...
public:
    ...
    // The derivative of the loss function is used to calculate the gradient of the network.
    // The loss function takes the output of the network and the expected output, which
    // is either a vector of floats that represent the correct model output, or a single
    // integer label that is the index of the correct model output.
    virtual std::vector<float> derivative(const std::vector<float>& output, const std::vector<float>& expected_output) const = 0;
    std::vector<float> derivative(const std::vector<float>& output, int expected_output_label) const;
};
...
public:
    ...
    using LossFunction::derivative;

    float operator()(const std::vector<float>& output, const std::vector<float>& expected_output) const override;

    std::vector<float> derivative(const std::vector<float>& output, const std::vector<float>& expected_output) const override;
};
